...
async def ai_food_lookup(request: Request) -> JSONResponse:
    ...
    if not _FOOD_QUERY_RE.match(query):
        return JSONResponse({"error": "Invalid characters in food name. Use only letters, numbers, spaces, and basic punctuation."}, status_code=400)
    ...
    results = {"query": query, "ai_estimate": None, "web_result": None}
    ...
    if provider != "stub" and base_url and model:
        try:
            ...
            from app.services import parse_json_object
    ...
    # 2. Web search via DuckDuckGo instant answer API (optional)
    if use_web:
        try:
    ...
    if not results["ai_estimate"] and not results["web_result"]:
        return JSONResponse({"error": "Could not estimate nutrition. Check that AI provider is configured in Settings."}, status_code=502)

    return JSONResponse(results)


@app.post("/api/ai-food-lookup/save")
async def ai_food_save_to_db(request: Request) -> JSONResponse:
