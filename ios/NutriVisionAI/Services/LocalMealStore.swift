...
final class LocalMealStore {
    ...
    // MARK: - Save (async — does not block the calling thread)

    func saveMeal(name: String, image: UIImage?, items: [AnalysisItem]) async -> Int {
        guard let db = db else { return -1 }
        return await withCheckedContinuation { continuation in
            queue.async { [imagesDir] in
                let mealId = Self._saveMealSync(db: db, imagesDir: imagesDir, name: name, image: image, items: items)
                continuation.resume(returning: mealId)
            }
        }
    }

    /// Synchronous core shared by the async wrapper. Runs on `queue`.
    private static func _saveMealSync(db: OpaquePointer, imagesDir: URL, name: String, image: UIImage?, items: [AnalysisItem]) -> Int {
        var mealId: Int = -1
        ...
        var stmt: OpaquePointer?
        guard sqlite3_prepare_v2(db, insertMeal, -1, &stmt, nil) == SQLITE_OK else { return mealId }
        ...
        guard sqlite3_step(stmt) == SQLITE_DONE else { return mealId }
    ...
    func recentMeals(limit: Int = 10) async -> [MealRecord] {
        guard let db = db else { return [] }
    ...
    func todaySummary() async -> DashboardSummary {
    ...
    func history(days: Int = 14) async -> HistoryResponse {
        ...
        return await withCheckedContinuation { continuation in
            queue.async {
                ...
                continuation.resume(returning: HistoryResponse(trends: trends, groupedMeals: groupedMeals, topFoods: topFoods))
            }
        }
    }
    ...
    func saveCustomFood(name: String, servingGrams: Double, calories: Double,
        ...
        return await withCheckedContinuation { continuation in
            queue.async {
                ...
                continuation.resume(returning: foodId)
            }
        }
    }

    func lookupByBarcode(_ barcode: String) async -> CustomFood? {
    ...
    func allCustomFoods() async -> [CustomFood] {
        guard let db = db else { return [] }
        return await withCheckedContinuation { continuation in
            queue.async {
    ...
    func deleteCustomFood(id: Int) async {
        guard let db = db else { return }
        await withCheckedContinuation { (continuation: CheckedContinuation<Void, Never>) in
            queue.async {
